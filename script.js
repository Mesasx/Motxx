    ...
    setTimeout(() => {
      addBotMessage(`
        <div class="bot-welcome-card">
          <span class="chat-budget-kicker">MotxxBot</span>
...
const INTENTS = [
  {
    ...
    reply: '¡Hola! Soy MotxxBot. Puedo hacerte un presupuesto aproximado en modo chat, sin formulario largo, o explicarte los cursos y automatizaciones.',
    quick: ['Presupuesto aproximado', 'Diagnóstico rápido', 'Ver cursos', 'Telegram']
  },
  {
    id: 'servicios',
    patterns: ['servicio', 'automatizar', 'automatizacion', 'automatización', 'que haceis', 'que ofreceis', 'que es motxx', 'agente', 'bot', 'ia para empresa'],
...
function showQuoteResult() {
  ...
  addBotMessage(`
    <div class="chat-budget-result quote-result-card">
      <span class="chat-budget-kicker">Estimación MotxxBot</span>
...
function submitChatQuote() {
  ...
  if (!form || !lastQuoteResult || !leadState) {
    addBotMessage('No he podido enviar el resumen automáticamente. Puedes escribirnos a contacto@motxx.es o usar el formulario clásico.');
  ...
  try {
    ...
    addBotMessage(`
      <div class="chat-budget-result quote-result-card">
        ...
        <p>Hemos enviado el resumen al equipo de Motxx IA. Te responderemos en menos de 24 horas hábiles con el siguiente paso.</p>
      </div>
    `);
    renderQuickReplies(['Telegram', '__quote_start__|Nueva estimación', 'Ver cursos']);
  } catch (e) {
    addBotMessage('Ha fallado el envío automático. Puedes usar el formulario clásico o escribir directamente a contacto@motxx.es.');
    renderQuickReplies(['Email clásico', 'Telegram']);
  }
}
...
function openSmartChatFromTopic(topic = 'general') {
  ...
  addUserMessage(topic === 'presupuesto' ? 'Presupuesto inteligente' : 'Diagnóstico MotxxBot');
...
function runSpecialAction(key, displayLabel) {
  ...
  switch (key) {
    ...
    case '__open_telegram__':
      ...
      addBotMessage(`Abriendo Telegram. También puedes entrar aquí: <a href="${CHATBOT.TELEGRAM}" target="_blank" rel="noopener">@MotxxBot</a>`);
      return true;
    case '__open_email__':
      addUserMessage(displayLabel || 'Email');
      window.location.href = CHATBOT.EMAIL;
      addBotMessage(`Abriendo tu correo. Dirección: <a href="${CHATBOT.EMAIL}">contacto@motxx.es</a>`);
      return true;
  }
  return false;
}
